...
export const calculateResult = (answers: number[], totalQuestions: number): ResultData => {
  ...
  const mbtiDescriptions: { [key: string]: string } = {
    ...
    'ENTJ': '(신비로운 사자) 당신은 진취적이고 대담한 통솔자입니다. 첫번째 강점은 통솔력, 두번째 강점은 통찰력, 약점은 공감력입니다. 목표를 향해 사람들을 이끌어나가는 리더십의 소유자로 왕의 자질을 가진 사람입니다. 왕 또는 소시오패스 위험군입니다.',
  };
  ...
  const charMtiCDescriptions: { [key: string]: string } = {
    'C': '(열렬히 박수받는 사람) 당신은 주도적인 Charm(매력형)입니다. 당신은 당당하고 자신감이 넘치는 주도적인 사람입니다. 어려운 상황에도 피하지 않고 나서서 사람들을 이끌어 나가는 주도적이고 매력적인 사람입니다. 인기쟁이 또는 바람둥이입니다.',
    'D': '(조용히 박수쳐주는 사람) 당신은 소극적인 Dawn(관망형)입니다. 당신은 조용하고 함부로 나서지 않는 신중한 사람입니다. 앞에 나서서 사람들의 관심을 받기보다 뒤에서 조용히 돕는, 겸손함과 배려심을 가진 사람입니다. 도우미 또는 찐따입니다.',
  };
