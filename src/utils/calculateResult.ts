...
export const calculateResult = (answers: number[], totalQuestions: number): ResultData => {
  ...
  const mbtiDescriptions: { [key: string]: string } = {
    'ISTJ': '(부지런한 개미) 당신은 성실하고 꼼꼼한 원칙주의자입니다. 첫번째 강점은 성실성, 두번째 강점은 통솔력, 약점은 창의력입니다. 자신에게 주어진 일을 섬세하게 잘 처리하는 사람으로서 조직생활에 잘 어울리는 사람입니다. 법없이 사는 사람 또는 꼰대입니다.',
    ...
    'INFJ': '(희생적인 까마귀) 당신은 따뜻하고 희생적인 예언자입니다. 첫번째 강점은 통찰력, 두번째 강점은 친화력, 약점은 추진력입니다. 깊은 통찰력으로 세상을 바라보며 다른 사람들의 감정을 잘 이해하는 사람입니다. 희생적인 자선가 또는 오지랖입니다.',
    'INTJ': '(민첩한 치타) 당신은 독립적이고 날카로운 전략가입니다. 첫번째 강점은 통찰력, 두번째 강점은 통솔력, 약점은 추진력입니다. 복잡한 문제를 분석하는 능력이 있으며, 목표를 향해 계획적으로 나아가는 사람입니다. 제갈공명형 전략가 또는 고집불통입니다.',
    'ISTP': '(느긋한 나무늘보) 당신은 여유롭고 섬세한 기술자입니다. 첫번째 강점은 분석력, 두번째 강점은 추진력, 약점은 친화력입니다. 조용히 지내며 간섭하지도 간섭받지도 않는 사람으로 문제해결 능력이 뛰어납니다. 최고의 기술장인 또는 나만 아니면 돼~ 주의자입니다.',
    ...
    'ESFJ': '(평화의 비둘기) 당신은 협력적이고 사교적인 외교관입니다. 첫번째 강점은 친화력, 두번째 강점은 성실성, 약점은 분석력입니다. 사람들을 좋아하고 그들을 서로 연결하는 일을 잘하며, 적이 없이 두루두루 인기가 많은 사람입니다. 인기쟁이 또는 설레발입니다.',
    'ENFJ': '(정의로운 독수리) 당신은 다정하고 정의로운 사회운동가입니다. 첫번째 강점은 친화력, 두번째 강점은 통찰력, 약점은 분석력입니다. 다정한 성격으로 욕심이 없으며, 헌신적으로 사람들을 이끄는 능력이 뛰어난 사람입니다. 지도자 또는 선동가입니다.',
    'ENTJ': '(신비로운 사자) 당신은 진취적이고 대담한 통솔자입니다. 첫번째 강점은 통솔력, 두번째 강점은 통찰력, 약점은 공감력입니다. 목표를 향해 사람들을 이끌어나가는, 탁월한 리더십이 있는 왕의 자질을 가진 사람입니다. 임금님 또는 소시오패스 위험군입니다.',
  };

  // CharMTI A/B 유형 설명
  const charMtiADescriptions: { [key: string]: string } = {
    'A': '(고요한 바다) 당신은 감정의 동요가 적은 Atlas(안정형)입니다. 차분하고 평온하며, 어떤 상황에서도 쉽게 흔들리지 않는 강철 같은 마음을 가지고 있습니다. 주변에 편안함과 안정감을 주는 존재입니다.',
    'B': '(타오르는 불꽃) 당신은 감정 표현이 풍부한 Blaze(열정형)입니다. 때로는 예민하며 감정 기복이 있을 수 있지만, 이는 곧 당신의 활기찬 에너지를 보여주는 증거입니다. 당신의 열정은 주변을 움직이는 원동력이 됩니다.',
  };

  // CharMTI C/D 유형 설명
  const charMtiCDescriptions: { [key: string]: string } = {
    'C': '(열렬히 박수받는 사람) 당신은 주도적인 Charm(매력형)입니다. 당당하고 자신감이 넘치며, 사람들의 이목을 집중시키는 특별한 매력을 가지고 있습니다. 어떤 상황에서도 주도적으로 나서며 사람들을 이끌어 나가는 성향입니다. 인기쟁이 또는 바람둥이입니다.',
    'D': '(조용히 박수쳐주는 사람) 당신은 소극적인 Dawn(관망형)입니다. 주저하고 자신감이 부족하지만, 차분함과 겸손함과 배려심을 가지고 있습니다. 함부로 나서지 않고 뒤에서 조용히 사람들을 돕는 성향입니다. 헌신적인 도우미 또는 찐따입니다.',
  };
