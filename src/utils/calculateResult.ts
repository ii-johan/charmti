...
export const calculateResult = (answers: number[], totalQuestions: number): ResultData => {
  ...
  const mbtiDescriptions: { [key: string]: string } = {
    ...
    'ISFJ': '(이타적인 사슴) 당신은 헌신적인 수호자입니다. 첫번째 강점은 성실성, 두번째 강점은 친화력, 약점은 창의력입니다. 다른 사람 돕는 것을 좋아하며, 조용하지만 강한 영향력을 행사하는 사람입니다. 헌신적인 봉사자 또는 호구입니다.',
    ...
    'ISFP': '(온순한 고양이) 당신은 감성이 풍부한, 자유로운 영혼의 예술가입니다. 첫번째 장점은 공감력, 두번째 장점은 추진력, 약점은 통솔력입니다. 예술적 감각이 뛰어나며 자유로운 삶을 추구하는 사람입니다. 예술가 또는 회피적 방랑자입니다.',
  ...
  const charMtiCDescriptions: { [key: string]: string } = {
    ...
    'D': '(조용히 박수치는 사람) 당신은 소극적인 Dawn(관망형)입니다. 주저하고 자신감이 부족하지만, 차분함과 겸손함과 배려심을 가지고 있습니다. 함부로 나서지 않고 뒤에서 조용히 사람들을 돕는 성향입니다. 헌신적인 도우미 또는 찐따입니다.',
  };
